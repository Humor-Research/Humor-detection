...
for dataset in tqdm(HRI_PAPER_2023_DATASETS):
    ...
    result.append(
        (dataset, f1_score(hri_dataset.get_test()["label"], preds_label), recall_score(hri_dataset.get_test()["label"], preds_label))
    )


df = pd.DataFrame(result, columns=['dataset', 'f1_score', 'recall'])
df.to_csv("./reports/colbert_pred_hd.csv")


result = list()

for dataset in tqdm(SUPPORTED_CONVERSATIONAL_DATASETS):
    hri_dataset = ConversationalDataset(dataset)
    test_inputs = compute_input_arrays(
        hri_dataset.get_data(),
        input_categories,
        tokenizer
    )
    test_preds = model.predict(test_inputs)
    ...
    ratio = sum(preds_label) / len(preds_label)
...
df = pd.DataFrame(result, columns=['dataset', 'ratio'])
df.to_csv("./reports_colbert/colbert_pred_cd.csv")