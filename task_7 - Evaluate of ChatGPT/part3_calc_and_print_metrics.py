...
def normalize_preds(raw_pred):
    ...
    dict_pred = json.loads(raw_pred)
    pred = dict_pred["choices"][0]["message"]["content"]
    pred = str(pred).lower()
    pred = pred.replace(".", "")
    pred = pred.replace(",", "")
...
df["chatgpt_preds"] = df["chatgpt_preds"].apply(normalize_preds)
...
for d_name in all_datasets:
    ...
    metrics_dict[metrics_index] = {
        "dataset": d_name,
        "model": "chatgpt",
        "f1_score": f1_score(tmp_df["label"], tmp_df["chatgpt_preds"]),
        "recall_score": recall_score(tmp_df["label"], tmp_df["chatgpt_preds"])
    }
...
for d_name in all_datasets:
    ...
    metrics_dict[metrics_index] = {
        "dataset": d_name,
        "model": "chatgpt",
        "ratio_score": tmp_df["chatgpt_preds"].sum() / len(tmp_df),
    }
