...
while test_data["chatgpt_preds"].isna().sum() != 0:
    ...
    test_data[["dataset", "label", "chatgpt_preds"]].to_csv(f"./reports_hd/{dataset}_pred_chatgpt.csv")

    time.sleep(1)

test_data = test_data[["dataset", "label", "chatgpt_preds"]]
test_data.to_csv(f"./reports_hd/{dataset}_pred_chatgpt.csv")
